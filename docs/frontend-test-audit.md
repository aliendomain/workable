...
| `GET /api/auth/entra/callback` | `src/app/api/auth/entra/callback/route.ts` | Partially Covered | A signed-token callback test covers metadata, token exchange, JWKS validation, successful session creation, cookie cleanup, and rejection of a tampered cross-origin return path. The thin route wrapper and callback failure branches are not directly covered. | Add callback tests for bad state/nonce, token exchange failure, disallowed users, target-token cookies, and the route wrapper. |
...
| Iteration detail route/view | `IterationConsoleView` | Partially Covered | Helper coverage remains strong, and mounted coverage now proves the iteration overview landing request plus summary/input/output/profile rendering from that payload. The standalone profile panel also has focused DOM coverage for summary pills, fixed-width method scope selection, detailed auto-expand, and hotspot/ancestor controls. Error/empty states, breadcrumb navigation, message filters, and deeper log interactions are still thin. | Component tests for error/empty states, breadcrumb definition link, message filters, and richer log interactions/load-more paths. |
...
| Workable proxy/security server helpers | `lib/admin-security*`, `lib/workable-proxy.ts` | Covered | Meaningful security/proxy tests cover default-deny auth, sessions, Basic/Entra provider selection, Entra authorization start and signed-token callback completion, origin-safe return paths, target token forwarding/refresh, bounded request streams, allow-listing, CSRF, TLS guidance, and hostile realtime metadata. | Add the remaining Entra callback failure branches and route-wrapper tests if handler logic changes. |
...
| 8 | Sign-out and auth/proxy route behavior | Partially Covered | Sign-out, login/logout route wrappers, public proxy routes, page redirects, API JSON failures, Basic pass-through, and signed-token Entra callback completion with a hostile return cookie are covered. Session-renewal proxy cookies, matcher exclusions, the Entra callback route wrapper, and callback error branches remain. | Yes, if auth/proxy is touched | Add session-renewal cookie and matcher tests for `proxy()`, plus Entra callback error and route-wrapper tests. |
...
| Medium | Empty/no-access server state | Saved-host/no-access variant is not mounted. | Main console test with saved host whose current user no longer has access to any discovered system; assert no-access messaging and Add server action. | Deferred | Lower refactor risk after no-work-access permission coverage; promote only if empty-state or host discovery filtering changes. |
...
| Medium | Iteration detail route/view | Mounted populated iteration-overview behavior now exists, but error/empty states, breadcrumb navigation, message filters, and deeper log behaviors are still thin. | Component tests for error/empty iteration detail, breadcrumb definition link, message filters, and richer log interactions/load-more behavior. | Deferred | Promote before major iteration detail refactors; the landing-route and profile-panel safety net is now in place. |
...
| Medium | Auth/proxy route wrappers | Entra callback completion is covered at the security-helper boundary, while its route wrapper, callback errors, token route auth failure, `/api/workable` wrapper params, proxy session renewal, and matcher exclusions are not directly tested. | Route/proxy tests for callback errors and wrapper wiring, token route auth failures, GET/POST path forwarding, query preservation, session-renewal cookie, and static exclusions. | Deferred | Security helpers are strong; promote before auth/proxy refactors. |
...
2. Add DefinitionView and WorkerConsoleView mounted coverage before broad detail-screen refactors. IterationConsoleView now has basic mounted overview/profile coverage, so the next gap there is error/filter/log interaction coverage rather than a zero-coverage screen.
