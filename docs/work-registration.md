...
This keeps dependencies pointed in the right direction:

- feature DLLs register available work
- host apps decide which Workable systems exist
- the configured system controls how work is executed

Feature DLLs define work once and let the host attach that work to the appropriate Workable system.
...
Each system receives an isolated catalog. Systems may share the same application DI container.
...
For small work items, use delegate registration. For work that needs its own class, implement one of the executor interfaces and register that executor type. Executor classes can provide their registration metadata with `WorkMetadataAttribute`.
...
Work registrations are unbound by default. Unbound registrations are included in systems that accept work from feature assemblies.
...
- Startup work sources run after the catalog is ready.
- Work definitions expose a unique name, category, and optional description for host browse/query screens.
...
- A system can opt out of work from feature assemblies.
