...
`WorkEventStream` broadcasts events to matching active subscriptions. Selective subscriptions, including event-type filters, use bounded per-subscription buffers; unfiltered broad `DropOldest` subscriptions can share a bounded source-event cursor log to avoid per-subscriber write fanout. Subscriptions are removed when they are disposed or their reader is canceled.
