...
For configuration source order, precedence, and override rules that apply before these interactions, see [Work Configuration](README.md).

This page does not describe one standalone configuration object. It describes the behavior that appears when multiple configuration facets are combined.

## Settings

| Interaction | Settings involved | Description |
| --- | --- | --- |
| `Recurrence` and `Concurrency` | `WorkRecurrenceConfiguration`, `WorkCoordinationConfiguration.Concurrency` | Waiting recurring workers still hold concurrency capacity. |
| `Recurrence` and `TransientRetry` | `WorkRecurrenceConfiguration`, `WorkTransientRetryConfiguration` | Retry attempts happen inside the recurring worker lifecycle before recurrence decides whether to continue. |
| `Start` and `Recurrence` | `WorkStartConfiguration`, `WorkRecurrenceConfiguration` | Queue waits target worker lifecycle points, not single iterations. |
| `Start` and `Concurrency` | `WorkStartConfiguration`, `WorkCoordinationConfiguration.Concurrency` | Deferred start changes how long queueing waits before returning. |
| `Idempotency` and `Recurrence` | `WorkCoordinationConfiguration.Idempotency`, `WorkRecurrenceConfiguration` | Recurring workers keep their subject across iterations and can continue blocking duplicates while waiting. |
...
Because retries and recurrence both operate inside one worker lifecycle, `IWorkerHandle.WaitForCompletion(...)` does not return on the first failed attempt. It returns only when the worker reaches a terminal completion state.
...
The same is true for `IWorkerHandle.WaitForCompletion(...)`. A recurring worker handle can remain incomplete indefinitely until the worker leaves the recurring lifecycle.
