...
Use the [SQL Server schema CLI](../../../packages/extensions/sqlserver/README.md#schema-cli) to generate or apply that script. With automatic deployment disabled, Workable validates the installed tables, columns, indexes, and component version and fails startup when the diagnostics schema is missing or incomplete.
