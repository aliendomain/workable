...
Keep the largest successfully applied `Sequence` as the resume cursor. After reconnecting, call `Subscribe(...)` or `StreamIterationStatus` with that value as `afterSequence`.
...
- SignalR emits one terminal `WorkableRealtimeIterationStatusMessage` with `Kind == "gap"`, a structured `Gap` value containing the same range, and no `Status`, then completes normally
...
The final `WorkOutput` remains the right place for a durable result. If a client must rebuild a conversation or operation after a process restart, persist that application state separately until durable status-stream storage is explicitly added.
...
Iteration status replay is in-memory and non-durable. It records publications before any subscriber arrives, uses system-configured replay and payload limits, follows existing worker retention for cleanup, and reports SignalR replay gaps as typed terminal messages. SQL persistence is intentionally outside this contract.
