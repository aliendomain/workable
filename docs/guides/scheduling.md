...
A host can instead register its own `IWorkScheduleStore`. Enabling scheduling without a registered store fails system startup rather than silently accepting non-durable schedules.
...
When `RunMissedExecution` is `true`, startup dispatches one overdue occurrence and advances a recurring schedule directly to its next future interval or cron occurrence. It does not emit a burst for every elapsed occurrence. When the option is `false`, that overdue occurrence is recorded as `Skipped` and the same advancement rule applies. The startup boundary is captured only after the work system has finished initializing and is ready to begin scheduler polling, so work that becomes due during a long startup is treated consistently as missed.
