...
This admin application's OAuth client is separate from the `Workable.Entra` host package. Its session, OAuth transaction, and delegated refresh authority use host-only `__Host-` cookies on HTTPS and reject ambiguous duplicates. Delegated access tokens are not stored in browser cookies or Web Storage; they use a bounded, scope-keyed, rotation-aware per-process cache with a per-token size limit, whose loss only requires another refresh-token exchange. Compact encrypted refresh snapshots are bound to the immutable validated Entra subject and individual admin session, cannot outlive that session's absolute lifetime, preserve rotation ordering without requiring a shared cache, and reject unsupported state versions. Rejected snapshots are cleared and automatically send the browser through sign-in again; pre-v1 snapshots are intentionally invalidated instead of migrated. Production delegated target APIs must use HTTPS. JSON backchannel responses are size- and time-bounded, redirects are refused, and discovery-provided token and signing-key endpoints must remain on the configured HTTPS authority origin. Each logout adds a signed immutable tombstone, invalidates that session's local access-token cache, and prevents an in-flight refresh from restoring the cache entry. Sessions and OAuth transactions retain the bounded tombstone snapshot they observed and are rejected when a later tombstone appears, including when an Entra callback was bound to a pre-logout transaction. Login and callback responses never write tombstones, concurrent logouts are additive, and a later logout compacts the observed set. Expired older tombstones can disappear without invalidating sessions created after them. See the [admin UI authentication guide](../../apps/web/workable-admin-ui/README.md#authentication).
