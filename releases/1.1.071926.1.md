...
- Transient durable auto-resume failures now enter one coalesced retry loop per blocked run with capped exponential backoff, allowing a later persistence attempt to resume the workflow exactly once while shutdown cancels and drains pending retries.
...
- Accepted workflow cancellation is now a monotonic control intent: a later pause request cannot overwrite a pending durable cancellation while the active execution and its children are still settling.
