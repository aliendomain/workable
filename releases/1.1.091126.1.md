...
- When the 1,000-row recent-history window is full, the schedule screen follows bounded continuation pages for the active view so older active schedules cannot be hidden by newer completed or canceled records or by an active limit above 1,000.
- One-time schedule copy now states that completion follows the first scheduled dispatch attempt, including a rejected queue admission.
...
- Panel-owned view actions remain inside their clipped viewport boundary, keeping **New schedule** fully visible below the console header and its menus.

## Public API and Configuration

- Added `IWorkScheduler` to work systems and caller-scoped sessions, together with schedule identifiers, timing, request, snapshot, cursor-paginated query, occurrence, creation, and cancellation contracts.
...
- The active-list keyset index keeps later pages comparable to the first: the focused query suite measured recent/active/continued-active pages at 33.36/27.96/26.91 ms with 1,000 retained schedules and 44.78/43.33/41.23 ms with 10,000 retained schedules.
...
- The final validation passed 2,499 core/API tests, 193 SQL Server tests, and 447 admin UI tests. Core/API branch coverage measured 96.48 percent, SQL Server source-condition coverage measured 95.42 percent, and the changed schedule screen measured 95.74 percent branch coverage. The SQL suite includes 50 synchronized create-versus-cleanup races that verify the shared lock order under contention.
