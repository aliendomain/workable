...
- Delegated Entra refresh authority is encrypted in compact, immutable browser snapshots and bound to the immutable signed-in identity, admin session, tenant/client, and target API configuration. Access tokens use a bounded, scope-keyed per-process cache with per-token size limits; cache loss falls back to refresh authority, legacy access-token-bearing cookies migrate on use, and refresh coordination supports concurrent requests and multiple admin UI processes.
