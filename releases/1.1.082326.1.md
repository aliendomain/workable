...
- Entra OAuth state is authenticated, production OAuth and session cookies use browser-enforced `__Host-` semantics, and duplicate cookies are rejected. Signed immutable logout tombstones prevent delayed Basic or Entra responses from restoring a logged-out session without shortening later sessions when older tombstones expire. Concurrent logouts are additive, later logouts compact the bounded set, and the common request verifies one tombstone without shared state or network work.
