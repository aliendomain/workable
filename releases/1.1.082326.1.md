...
- Delegated Entra refresh authority is encrypted in compact, immutable, versioned browser snapshots and bound to the immutable signed-in identity, admin session, tenant/client, and target API configuration. Access tokens use a bounded, scope-keyed, rotation-aware per-process cache with per-token size limits; cache loss falls back to refresh authority, and one signed session-bound coordinator serializes concurrent refreshes within each admin UI process. Unsupported and pre-v1 snapshots are cleared and automatically send the browser through sign-in again rather than leaving the current page in repeated authorization failures.
