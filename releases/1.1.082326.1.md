...
- Session-signing secrets must contain at least 32 UTF-8 bytes. Sessions are bound to the active Basic credentials or Entra tenant/client configuration, re-evaluate Entra email policy on every request, and enforce an absolute lifetime in addition to sliding renewal.
- Entra OAuth state is authenticated, production OAuth and session cookies use browser-enforced `__Host-` semantics, duplicate cookies are rejected, and a signed logout generation prevents delayed responses from restoring a logged-out session.
- Entra metadata, signing-key, and token backchannels enforce HTTPS authority-origin binding, refused redirects, request deadlines, cancellation, one-MiB response limits, and bounded coalescing caches.
