...
The checked-in `workable-sqlserver-schema.sql` is generated from the same schema source used by runtime auto-deployment and includes every component required by this sample, including scheduling. A container reused from pre-release scheduling development may contain a draft `Scheduling` version marker with incomplete tables; runtime validation intentionally rejects that inconsistent state. Because the managed sample database is disposable, remove the `workable-samplehost-sqlserver` container to recreate it cleanly. For a database whose contents must be retained, inspect and repair the inconsistent draft schema explicitly instead of bypassing validation.
