...
Queue `sample.demo.assistant-stream` to exercise iteration status streaming. It emits `assistant.message.started`, word-sized `assistant.text.delta` items, and `assistant.message.completed`, with a configurable delay between chunks. Its final output contains the assembled response. Consume the live items through the SignalR `StreamIterationStatus` hub method using the worker id and current iteration sequence returned by the worker query surface. See the [Iteration Status Streams guide](../../../docs/guides/iteration-status-streams.md) for .NET and JavaScript clients.
...
Runtime scheduling is enabled for both the default Operations system and the named Fulfillment system using the same SQL Server persistence target. In the admin UI, open a system's **Schedules** tree item to inspect upcoming work, review dispatch history, create a schedule, or cancel one. You can also open **Catalog**, select a work definition, click **Queue**, and use **Schedule** in the dialog footer. Recurring definitions can be scheduled once but cannot create another recurring schedule.

The checked-in `workable-sqlserver-schema.sql` is generated from the same schema source used by runtime auto-deployment and includes every component required by this sample, including scheduling. Scheduling schema version 2 adds the active-list index used by the admin overview; runtime auto-deployment upgrades a complete version-1 scheduling schema in place without removing schedules. Runtime validation still rejects an unversioned or otherwise incomplete draft schema. Because the managed sample database is disposable, remove the `workable-samplehost-sqlserver` container if such a draft must be recreated cleanly. For a database whose contents must be retained, inspect and repair the inconsistent draft schema explicitly instead of bypassing validation.
