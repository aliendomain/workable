...
The same capability adds **Schedules** after **Iterations** in each enabled system's explorer tree. That screen separates upcoming work from retained schedule history, shows dispatch history and accepted worker links for the selected schedule, and supports cancellation. One overview request returns the first 25 recent schedules, the first 25 upcoming schedules in due-time order, exact visible active/upcoming/recurring counts, the selected retained schedule, and its dispatch history. Each list uses a fixed-height viewport with sticky headings and automatically continues its independent keyset as the user scrolls. Each expanded list keeps a sliding five-page, 125-record window in the browser so polling, rendering, and retained client state remain bounded; polls replace the current cursor-anchored windows with authoritative provider-ordered results, and **Return to newest** or **Return to soonest** clears an older window's anchor. The screen polls about every ten seconds while its tab is visible, prevents overlapping refreshes, applies randomized jitter, backs failures off to at most one retry per minute, and refreshes immediately when the tab becomes visible. **New schedule** opens a catalog picker and then opens the existing queue configuration dialog directly in schedule mode.
