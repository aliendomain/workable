...
The same capability adds **Schedules** after **Iterations** in each enabled system's explorer tree. That screen separates upcoming work from the 1,000 most recently created retained schedules, shows dispatch history and accepted worker links for the selected schedule, and supports cancellation. If retained history fills the recent window, the UI also requests the bounded active-schedule view so older active schedules remain visible and cancelable. **New schedule** opens a catalog picker and then opens the existing queue configuration dialog directly in schedule mode.
