...
- `BaselineSqlScheduleQueryBenchmarks` measures the maximum 1,000-row initial and continued schedule-list provider surfaces plus the administration UI's consolidated 100-row overview command. `BaselineSqlScheduleOccurrenceQueryBenchmarks` measures bounded occurrence-history reads at configured retention scales.
...
The consolidated management overview now has its own benchmark at the UI's 100-row recent and upcoming page sizes. After restricting exact metric aggregation to active rows, it measured 102.48 ms and 593.82 KiB with 1,000 retained schedules, and 124.29 ms and 569.30 KiB with 10,000 retained schedules. The same final run measured recent/active/continued-active maximum pages at 27.98/26.55/23.50 ms with 1,000 retained schedules and 41.61/36.27/37.96 ms with 10,000 retained schedules.
