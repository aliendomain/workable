...
public sealed class WorkQueryServiceTests
{
    ...
    public async Task WorkersQueryFiltersByOriginActorId()
    {
        ...
        var firstSession = await system.CreateSession(WorkRequestContext.Create(WorkInvocationChannel.InProcess, firstActor));
        var secondSession = await system.CreateSession(WorkRequestContext.Create(WorkInvocationChannel.InProcess, secondActor));
    ...
    [Fact]
    public async Task ReadModelEventuallyPublishesWhenProjectorBatchExactlyDrains()
    {
